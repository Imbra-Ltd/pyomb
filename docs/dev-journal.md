...
## 2026-09-07 -- Groom the backlog, answer instead of dropping

- **Tool:** Claude Code (Opus 5, 1M context).
- **Key changes:**
  - **Groomed all fourteen open issues against the tree.** Seven carried a
    premise that had moved and now carry a dated correction. #149 argues for a
    complexity ratchet from a legacy surface the ruff freeze recorded, and
    ADR-047 and ADR-048 retired both freezes, so whether `complexipy` needs a
    baseline at all is now an unmeasured question and its fifth acceptance
    criterion names two records that no longer say what it cites. #217 asks for
    a check that already exists: `base-git` names a release-ordering check,
    bound here since the v2.61.0 pin on 2026-08-28, one day before the issue
    was filed -- so the work is adoption rather than invention, and PLAYBOOK 5
    still tells an operator nothing enforces the rule. #318's table listed one
    command that left the playbook with the ruff freeze. #210's tests moved to
    `tests/integration/` in #317, so a bare `pytest` no longer shows the
    warning at all. #334, #304 and #311 were re-measured or re-verified.
    #353 and #352 carried a type label and no priority, which the label
    conformance check reports; both are now P2.
  - **Answered a parse failure instead of dropping the peer (PR #355).**
    `ModbusTcpRequest.deserialize` read the MBAP header, checked its length
    field and parsed the PDU inside one `try`, so all three failures reached
    the caller as the same `ModbusPacketError` and a server could not tell the
    answerable case from the others. A PDU that fails behind a header that
    parsed now raises `ModbusPduParseError` carrying that header and the
    function code, and the simulator replies with exception code 0x03 and
    keeps the connection. ADR-049 records which failures are answerable and
    which are not.
  - **Dispatched on the request class rather than the function code
    (PR #356).** `ModbusPduParser.register` is public and documented as the
    way to model a PDU shape this library does not carry, and registering
    against a code the server already answers dropped the connection: the
    branch tested a field, the factory read a field only the built-in class
    declares, and the registry is what a caller had just retied. Twelve arms
    now test `isinstance`, which is the narrowing each factory needs, so all
    eleven casts and the `cast` import are gone. ADR-050 records it.
  - **Named `checks/` in every documented lint and scan command (PR #357).**
    Nine commands across `CLAUDE.md`, the playbook and the onboarding guide
    scanned eighteen gate modules fewer than CI does, and both runs printed
    `All checks passed!`. The second acceptance criterion asked whether a gate
    should compare the two copies;
    `checks/test_documented_commands_match_the_pipeline.py` does, reading
    every fenced command in those documents and comparing its directory set
    against its pipeline step's.
- **PRs merged:** none. #355, #356 and #357 are open, green and CLEAN. #356 is
  stacked on #355, so the order is forced and a delete-branch flag must not be
  passed while it targets that branch.
- **Issues closed/created:** closed none -- the three worked issues close on
  merge. Created #358 (a merged record argues from RTU timing an
  implementation cannot observe) and #359 (correcting a factual error in a
  merged record costs a whole record).
- **Lesson:** a test can assert a behaviour the wire cannot produce. The drop
  half of the answer-or-drop split was asserted by sending four bytes over a
  real socket and expecting the peer to be retired. That frame never reaches
  `on_data`: the transport waits for all seven header bytes before it reads
  anything else, so the connection sits waiting. It passed on Windows because
  the client's own `close()` retired the peer inside the settle window, and
  failed on Linux. The assertion was measuring the teardown. Injected against
  a connection double instead, with a length field contradicting the ADU,
  which a peer can genuinely deliver.
- **Lesson:** an upstream implementation is worth reading before designing
  against a specification. #231 and #311 both rested on RTU frames being split
  by measuring silence on the line, which is what the specification describes.
  pymodbus refuses to use timing and says why in its framer -- the USB
  converter and the OS drivers make it unguaranteeable in both directions --
  and splits by decoding the frame's content for its expected length, then
  verifying the checksum at that boundary. The correction makes that layer
  smaller and removes the reason #311 expected the first slow test.
- **Lesson:** a falsified premise can sit inside a merged record. ADR-035 rule
  1 refuses to split `packets.py` and attaches a revisit trigger, and the
  reason it gives is that an RTU framer splits on silence where a TCP framer
  splits on a declared length. The clause is true of the wire and false of
  what a program can observe, and what breaks is the inference: two
  content-based framers are more alike than the record claims. The trigger has
  not fired, so #358 carries the evidence rather than spending a record on it.
- **Lesson:** the record set already holds the failure #359 asks about.
  ADR-013 corrected a measurement in ADR-012 and marked it `Superseded`, and
  its own text says the supersession is on the evidence and not on the design.
  Four of ADR-012's five decisions are current, six documents cite it in
  prose, and every one of those pointers resolves to something labelled dead.
- **Upstream:** none filed. ADR-049 and ADR-050 both record `none` with the
  reason at decision time: the first draws its cut from one wire format's
  framing rules, and the second's rule is general but its findable tell is a
  cast per branch, which a template rule would have to carry to be useful.
- **Not done:** the bind-collision warnings #210 records are still there in
  the integration tier. This session did not reach them.
- **Pending:** the submodule pin sits at `v2.79.0` against `v2.83.0`, four
  releases behind. It is off-limits, so the bump needs a proposal carrying a
  rollback strategy, and it owes a re-read of the recorded divergences before
  the diff is read as a gap to close. Carried from the previous entry, three
  releases wider.
