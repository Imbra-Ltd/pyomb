...
- A comment block is at most 2 lines, and only where the intent cannot be
  expressed in code. This binds commented configuration as well as source. A
  licence header is exempt, and a label trailing code is not a block
...
- Both are for a junior reader, so prose that will not fit moves to
  ...
  directories the migration in the tracked issue has reached and whose
  `CONFIG` names the suffixes that put every tracked manifest and workflow
  under the comment bound
