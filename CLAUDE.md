...
- A comment block is at most 2 lines, and only where the intent cannot be
  expressed in code. This binds commented configuration as well as source.
  Four things are outside it, each because it states no reasoning the bound
  could relocate; PLAYBOOK 3.26 names them
...
- Both are for a junior reader, so prose that will not fit moves to
  ...
  place. Enforced by `checks/test_comment_length.py`, whose `ROOTS` now names
  every Python directory in the tree and whose `CONFIG` names the suffixes
  that put every tracked manifest and workflow under the comment bound
